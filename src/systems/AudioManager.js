...
const VOL_BGM    = 0.13;  // background music — well under SFX
...
const VOL_HURT   = 0.78;  // damage hit — sharp and urgent
const VOL_SONAR  = 0.60;  // echolocation ping
const VOL_WHOOSH = 0.50;  // tutorial page transition

// ── Fade durations (seconds) ──────────────────────────────────────────────
const BGM_FADE_IN    = 1.80;  // slow atmospheric rise when level starts
const BGM_FADE_OUT   = 1.20;  // smooth fade on death / level complete
const WINGS_FADE_IN  = 0.12;
...
class AudioManager {
  ...
  _fetch(url, key) {
    fetch(url)
      ...
      .then(ab => { this._pending[key] = ab; this._tryDecode(key); })
  ...
  unlock() {
    ...
    for (const key of ['bgm', 'wings', 'walk', 'crunch', 'hurt', 'sonar', 'whoosh']) this._tryDecode(key);
  }
  ...
  // One-shot SFX.
  playCrunch() { this._playOneShot(this._crunchBuf, VOL_CRUNCH); }
  playHurt()   { this._playOneShot(this._hurtBuf,   VOL_HURT);   }
  playSonar()  { this._playOneShot(this._sonarBuf,  VOL_SONAR);  }
  playWhoosh() { this._playOneShot(this._whooshBuf, VOL_WHOOSH); }
  ...
  update(player) {
    ...
    const wantsWalk = !player.dead && player.onGround && moving && !player.isFlying;
